...
import { ExcalidrawElementSkeleton, convertToExcalidrawElements } from "./transform";
...
describe("Test Transform", () => {
	...
	it("should transform linear elements", () => {
		...
		const excaldrawElements = convertToExcalidrawElements(elements as ExcalidrawElementSkeleton[], opts);
	...
	describe("Test Frames", () => {
		it("should transform frames and update frame ids when regenerated", () => {
			...
			const excaldrawElements = convertToExcalidrawElements(elementsSkeleton, opts);
			expect(excaldrawElements.length).toBe(4);
	...
	describe("Test arrow bindings", () => {
		...
		it("should bind arrows to existing elements if ids are correct", () => {
			...
			expect(consoleErrorSpy).toHaveBeenNthCalledWith(2, "No element for end binding with id rect-11 found");
		});
