...
const _ce = ({ x, y, w, h, a, t }: { x: number; y: number; w: number; h: number; a?: number; t?: string }) =>
	({
		type: t || "rectangle",
		strokeColor: "#000",
		backgroundColor: "#000",
...
describe("getElementAbsoluteCoords", () => {
	...
	it("test x2 coordinate", () => {
		const [, , x2] = getElementAbsoluteCoords(_ce({ x: 10, y: 0, w: 10, h: 0 }));
	...
	it("test y2 coordinate", () => {
		const [, , , y2] = getElementAbsoluteCoords(_ce({ x: 0, y: 10, w: 0, h: 10 }));
