...
describe("<Excalidraw/>", () => {
	...
	describe("Test UIOptions prop", () => {
		describe("Test canvasActions", () => {
			...
			it("should hide export button when export is false", async () => {
				const { container } = await render(<Excalidraw UIOptions={{ canvasActions: { export: false } }} />);
				//open menu
				toggleMenu(container);
	...
	describe("<MainMenu/>", () => {
		...
		it("should update themeToggle text even if MainMenu memoized", async () => {
			...
			expect(queryByTestId(container, "toggle-dark-mode")?.textContent).toContain(t("buttons.darkMode"));

			fireEvent.click(queryByTestId(container, "toggle-dark-mode")!);

			expect(queryByTestId(container, "toggle-dark-mode")?.textContent).toContain(t("buttons.lightMode"));
		});
	});
});
