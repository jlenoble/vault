...
export class ActionManager {
	...
	handleKeyDown(event: React.KeyboardEvent | KeyboardEvent) {
		...
		const data = Object.values(this.actions)
			...
			.filter(
				(action) =>
					(action.name in canvasActions ? canvasActions[action.name as keyof typeof canvasActions] : true) &&
					action.keyTest &&
	...
	renderAction = (name: ActionName, data?: PanelComponentProps["data"]) => {
		...
		if (
			...
			const updateData = (formState?: any) => {
				...
				this.updater(
					action.perform(this.getElementsIncludingDeleted(), this.getAppState(), formState, this.app),
				);
			};
