...
import { BinaryFileData, BinaryFileMetadata, ExcalidrawImperativeAPI, BinaryFiles } from "../../src/types";
...
export class FileManager {
	...
	saveFiles = async ({ elements, files }: { elements: readonly ExcalidrawElement[]; files: BinaryFiles }) => {
	...
	shouldUpdateImageElementStatus = (
		...
		return (
			isInitializedImageElement(element) && this.isFileSaved(element.fileId) && element.status === "pending"
		);
	};
