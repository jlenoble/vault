...
const restoreElement = (
	...
	switch (element.type) {
		...
		case "arrow": {
			...
			return restoreElementWithProperties(element, {
				type: (element.type as ExcalidrawElement["type"] | "draw") === "draw" ? "line" : element.type,
...
export const restoreElements = (
	...
	const restoredElements = (elements || []).reduce((elements, element) => {
		...
		if (element.type !== "selection" && !isInvisiblySmallElement(element)) {
			let migratedElement: ExcalidrawElement | null = restoreElement(element, opts?.refreshDimensions);
			if (migratedElement) {
