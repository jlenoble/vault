...
import { getBoundTextElementPosition, wrapText, getBoundTextMaxWidth } from "../element/textElement";
...
describe("Test Linear Elements", () => {
	...
	const enterLineEditingMode = (line: ExcalidrawLinearElement, selectProgrammatically = false) => {
	...
	it("should allow entering and exiting line editor via context menu", () => {
		...
		fireEvent.click(queryByText(contextMenu as HTMLElement, "Exit line editor")!);
	...
	describe("Inside editor", () => {
      ...
      `);
		...
		it("should update the midpoints when element roundness changed", async () => {
			...
			const midPointsWithSharpEdge = LinearElementEditor.getEditorMidPoints(line, h.state);
      ...
      `);
		...
		it("should update all the midpoints when element position changed", async () => {
			...
			expect([line.x, line.y]).toEqual([points[0][0] + deltaX, points[0][1] + deltaY]);

			const newMidPoints = LinearElementEditor.getEditorMidPoints(line, h.state);
      ...
      `);
		...
		describe("When edges are round", () => {
			...
			it("should allow dragging lines from midpoints in between segments", async () => {
				...
				drag(firstSegmentMidpoint, [firstSegmentMidpoint[0] + delta, firstSegmentMidpoint[1] + delta]);
				expect(line.points.length).toEqual(4);

				// drag line from last segment midpoint
				drag(lastSegmentMidpoint, [lastSegmentMidpoint[0] + delta, lastSegmentMidpoint[1] + delta]);
				...
				expect((h.elements[0] as ExcalidrawLinearElement).points).toMatchInlineSnapshot(`
            [
              [
                0,
                0,
          ...
          `);
			...
			it("should update only the first segment midpoint when its point is dragged", async () => {
				...
				expect([newPoints[0][0], newPoints[0][1]]).toEqual([points[0][0] - delta, points[0][1] - delta]);
			...
			it("should remove the midpoint when one of the points in the segment is deleted", async () => {
				...
				drag(lastSegmentMidpoint, [lastSegmentMidpoint[0] + 50, lastSegmentMidpoint[1] + 50]);
				expect(line.points.length).toEqual(4);
		...
		describe("When edges are round", () => {
			...
			// hence hardcoding it so if later some bug is introduced
			// this will fail and we can fix it
			const firstSegmentMidpoint: Point = [55.9697848965255, 47.442326230998205];
			const lastSegmentMidpoint: Point = [76.08587175006699, 43.294165939653226];
			let line: ExcalidrawLinearElement;
	...
	describe("Test bound text element", () => {
		...
		const createBoundTextElement = (text: string, container: ExcalidrawLinearElement) => {
		...
		describe("Test getBoundTextElementPosition", () => {
			it("should return correct position for 2 pointer arrow", () => {
				...
				const position = LinearElementEditor.getBoundTextElementPosition(container, textElement);
		...
		it("should bind text to arrow when double clicked", async () => {
			...
			editor.blur();
			expect(arrow.boundElements).toStrictEqual([{ id: text.id, type: "text" }]);
			expect((h.elements[1] as ExcalidrawTextElementWithContainer).text).toMatchInlineSnapshot(`
          "Online whiteboard 
          collaboration made 
          easy"
        `);
		...
		it("should bind text to arrow when clicked on arrow and enter pressed", async () => {
			...
			editor.blur();
			expect(arrow.boundElements).toStrictEqual([{ id: textElement.id, type: "text" }]);
			expect((h.elements[1] as ExcalidrawTextElementWithContainer).text).toMatchInlineSnapshot(`
          "Online whiteboard 
          collaboration made 
          easy"
        ...
        easy"
      `);
			expect(LinearElementEditor.getElementAbsoluteCoords(container, true)).toMatchInlineSnapshot(`
          [
            20,
            20,
            105,
        ...
        `);
			...
			expect({ width: container.width, height: container.height }).toMatchInlineSnapshot(`
          {
            "height": 130,
      ...
      `);
		...
		it("should wrap the bound text when arrow bound container moves", async () => {
			...
			expect(wrapText(textElement.originalText, font, getBoundTextMaxWidth(arrow))).toMatchInlineSnapshot(`
        "Online whiteboard 
        collaboration made easy"
      `);
			const handleBindTextResizeSpy = vi.spyOn(textElementUtils, "handleBindTextResize");
			...
			expect(rect.y).toBe(0);
			expect(handleBindTextResizeSpy).toHaveBeenCalledWith(h.elements[1], false);
			expect(wrapText(textElement.originalText, font, getBoundTextMaxWidth(arrow))).toMatchInlineSnapshot(`
        "Online whiteboard 
        collaboration made 
        easy"
      `);
		...
		it("should update label coords when a label binded via context menu is unbinded", async () => {
			...
			expect(container.boundElements).toStrictEqual([{ id: h.elements[1].id, type: "text" }]);
			expect(text.containerId).toBe(container.id);
			expect(text.verticalAlign).toBe(VERTICAL_ALIGN.MIDDLE);

			mouse.reset();
			mouse.clickAt(container.x + container.width / 2, container.y + container.height / 2);
			mouse.down();
			mouse.up();
