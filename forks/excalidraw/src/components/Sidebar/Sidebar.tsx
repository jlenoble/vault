...
export const SidebarInner = forwardRef(
	(
		...
		return (
			<Island {...rest} className={clsx("sidebar", { "sidebar--docked": docked }, className)} ref={islandRef}>
				<SidebarPropsContext.Provider value={headerPropsRef.current}>{children}</SidebarPropsContext.Provider>
			</Island>
		);
	},
);
		...
		useEffect(() => {
			if (
				...
				((!appState.openSidebar && refPrevOpenSidebar?.current?.name === props.name) ||
					...
					(appState.openSidebar?.name === props.name && refPrevOpenSidebar?.current?.name !== props.name) ||
