...
import { isEmbeddableElement, isFrameElement, isLinearElement } from "../element/typeChecks";
import { isEmbeddableOrLabel, createPlaceholderEmbeddableLabel } from "../element/embeddable";
...
const renderLinearPointHandles = (
	...
	const midPoints = LinearElementEditor.getEditorMidPoints(element, appState).filter(
...
const getNormalizedCanvasDimensions = (canvas: HTMLCanvasElement, scale: number): [number, number] => {
...
const _renderInteractiveScene = ({
	...
	const isFrameSelected = selectedElements.some((element) => isFrameElement(element));

	// Getting the element using LinearElementEditor during collab mismatches version - being one head of visible elements due to
	// ShapeCache returns empty hence making sure that we get the
	// correct element from visible elements
	if (selectedElements.length === 1 && appState.editingLinearElement?.elementId === selectedElements[0].id) {
		renderLinearPointHandles(context, appState, selectedElements[0] as NonDeleted<ExcalidrawLinearElement>);
	}
	...
	if (!appState.multiElement && !appState.editingLinearElement) {
		...
		if (
			...
			renderLinearPointHandles(context, appState, selectedElements[0] as ExcalidrawLinearElement);
		}
		const selectionColor = renderConfig.selectionColor || oc.black;

		if (showBoundingBox) {
			// Optimisation for finding quickly relevant element ids
			const locallySelectedIds = selectedElements.reduce((acc: Record<string, boolean>, element) => {
				acc[element.id] = true;
				return acc;
			}, {});
				...
				(
					...
					if (renderConfig.remoteSelectedElementIds[element.id]) {
						selectionColors.push(
							...renderConfig.remoteSelectedElementIds[element.id].map((socketId: string) => {
					...
					if (selectionColors.length) {
						const [elementX1, elementY1, elementX2, elementY2, cx, cy] = getElementAbsoluteCoords(
							element,
							true,
						);
						acc.push({
 ...
 * and other ui stuff.
 */
export const renderInteractiveScene = <U extends typeof _renderInteractiveScene, T extends boolean = false>(
 ...
 */
export const renderStaticScene = (renderConfig: StaticSceneRenderConfig, throttle?: boolean) => {
	if (throttle) {
