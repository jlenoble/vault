...
export const actionFinalize = register({
	...
	perform: (elements, appState, _, { interactiveCanvas, focusContainer, scene }) => {
		...
		const pendingImageElement =
			appState.pendingImageElementId && scene.getElement(appState.pendingImageElementId);
				...
				if (isLoop) {
					...
					mutateElement(multiPointElement, {
						points: linePoints.map((point, index) =>
							index === linePoints.length - 1 ? ([firstPoint[0], firstPoint[1]] as const) : point,
						),
					});
				}
			}
		...
		return {
			...
			appState: {
				...
				cursorButton: "up",
				activeTool:
					(appState.activeTool.locked || appState.activeTool.type === "freedraw") && multiPointElement
						? appState.activeTool
						: activeTool,
				...
				suggestedBindings: [],
				selectedElementIds:
					multiPointElement && !appState.activeTool.locked && appState.activeTool.type !== "freedraw"
						? {
