...
export const generateRoughOptions = (element: ExcalidrawElement, continuousPath = false): Options => {
	const options: Options = {
		...
		roughness: adjustRoughness(Math.min(element.width, element.height), element.roughness),
...
const modifyEmbeddableForRoughOptions = (element: NonDeletedExcalidrawElement, isExporting: boolean) => {
	if (
		...
		case "embeddable": {
			...
			if (element.roundness) {
				...
				shape = generator.path(
					...
					generateRoughOptions(modifyEmbeddableForRoughOptions(element, isExporting), true),
				);
			} else {
				shape = generator.rectangle(
					...
					element.width,
					element.height,
					generateRoughOptions(modifyEmbeddableForRoughOptions(element, isExporting), false),
				);
			}
			return shape;
		}
