...
describe("restoreElements", () => {
	...
	it("should restore freedraw element correctly", () => {
		...
		const restoredFreedraw = restore.restoreElements([freedrawElement], null)[0] as ExcalidrawFreeDrawElement;
...
describe("restoreAppState", () => {
	...
	it("should handle appState.openSidebar legacy values", () => {
		...
		expect(restore.restoreAppState({ openSidebar: { name: "library" } } as any, null).openSidebar).toEqual({
			name: "library",
		});
		expect(
			restore.restoreAppState({ openSidebar: { name: DEFAULT_SIDEBAR.name, tab: "ola" } } as any, null)
				.openSidebar,
...
describe("repairing bindings", () => {
	it("should repair container boundElements when repair is true", () => {
		...
		restoredElements = restore.restoreElements([container, boundElement], null, { repairBindings: true });
	...
	it("should remove bindings of deleted elements from boundElements when repair is true", () => {
		...
		let restoredElements = restore.restoreElements([container, invisibleBoundElement, boundElement], null);
		...
		restoredElements = restore.restoreElements([container, invisibleBoundElement, boundElement], null, {
			repairBindings: true,
		});
