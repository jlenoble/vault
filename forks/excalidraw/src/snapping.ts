...
export const isSnappingEnabled = ({
	...
	if (event) {
		return (
			...
			(!appState.objectsSnapModeEnabled && event[KEYS.CTRL_OR_CMD] && appState.gridSize === null)
		);
	}
...
const getGapSnaps = (
	...
	if (visibleGaps) {
		...
		const [minX, minY, maxX, maxY] = getDraggedElementsBounds(selectedElements, dragOffset).map((bound) =>
			round(bound),
		);
	...
	if (nearestSnapsX.length > 0) {
		for (const snap of nearestSnapsX) {
			if (snap.type === "point") {
				...
				snapsX[key].push(...snap.points.map((point) => [round(point[0]), round(point[1])] as Point));
			}
		}
	}
	...
	if (nearestSnapsY.length > 0) {
		for (const snap of nearestSnapsY) {
			if (snap.type === "point") {
				...
				snapsY[key].push(...snap.points.map((point) => [round(point[0]), round(point[1])] as Point));
			}
		}
	}
...
const createGapSnapLines = (
	...
	return dedupeGapSnapLines(
		gapSnapLines.map((gapSnapLine) => {
			return {
				...gapSnapLine,
				points: gapSnapLine.points.map((point) => [round(point[0]), round(point[1])] as Point) as PointPair,
			};
		}),
	);
};
...
export const snapNewElement = (
	...
	const selectionSnapPoints: Point[] = [[origin.x + dragOffset.x, origin.y + dragOffset.y]];
