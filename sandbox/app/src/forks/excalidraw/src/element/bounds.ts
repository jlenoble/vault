 ...
 */
export const getElementLineSegments = (element: ExcalidrawElement): [Point, Point][] => {
	const [x1, y1, x2, y2, cx, cy] = getElementAbsoluteCoords(element);
	...
	if (isLinearElement(element) || isFreeDrawElement(element)) {
		...
		while (i < element.points.length - 1) {
			segments.push([
				rotatePoint(
					[element.points[i][0] + element.x, element.points[i][1] + element.y] as Point,
					center,
					element.angle,
				),
				rotatePoint(
					[element.points[i + 1][0] + element.x, element.points[i + 1][1] + element.y] as Point,
					center,
					element.angle,
				),
			]);
...
export const pointRelativeTo = (element: ExcalidrawElement, absoluteCoords: Point): Point => {
...
const solveQuadratic = (p0: number, p1: number, p2: number, p3: number): MaybeQuadraticSolution => {
	const i = p1 - p0;
	const j = p2 - p1;
	const k = p3 - p2;
...
const getCubicBezierCurveBound = (p0: Point, p1: Point, p2: Point, p3: Point): Bounds => {
...
export const getBoundsFromPoints = (points: ExcalidrawFreeDrawElement["points"]): Bounds => {
	let minX = Infinity;
	let minY = Infinity;
	let maxX = -Infinity;
	let maxY = -Infinity;
...
const generateLinearElementShape = (element: ExcalidrawLinearElement): Drawable => {
...
const getLinearElementRotatedBounds = (element: ExcalidrawLinearElement, cx: number, cy: number): Bounds => {
	...
	const transformXY = (x: number, y: number) => rotate(element.x + x, element.y + y, cx, cy, element.angle);
...
export const getCommonBounds = (elements: readonly ExcalidrawElement[]): Bounds => {
	if (!elements.length) {
		return [0, 0, 0, 0];
	}
...
export const getResizedElementAbsoluteCoords = (
	...
	if (isFreeDrawElement(element)) {
		...
		const curve = !element.roundness
			? gen.linearPath(points as [number, number][], generateRoughOptions(element))
