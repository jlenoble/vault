...
export const shouldEnableBindingForPointerEvent = (event: React.PointerEvent<HTMLElement>) => {
	...
	getNonDeletedElements(Scene.getScene(linearElement)!, onlyUnbound).forEach((element) => {
		mutateElement(element, {
			boundElements: element.boundElements?.filter(
				(element) => element.type !== "arrow" || element.id !== linearElement.id,
			),
		});
	});
};
...
export const bindOrUnbindSelectedElements = (elements: NonDeleted<ExcalidrawElement>[]): void => {
...
const maybeBindBindableElement = (bindableElement: NonDeleted<ExcalidrawBindableElement>): void => {
	getElligibleElementsForBindableElementAndWhere(bindableElement).forEach(([linearElement, where]) =>
		bindOrUnbindLinearElement(
			linearElement,
...
export const unbindLinearElements = (elements: NonDeleted<ExcalidrawElement>[]): void => {
	elements.forEach((element) => {
		if (isBindingElement(element)) {
...
export const updateBoundElements = (
	...
	getNonDeletedElements(
		...
		updateBoundPoint(element, "start", startBinding, changedElement as ExcalidrawBindableElement);
		updateBoundPoint(element, "end", endBinding, changedElement as ExcalidrawBindableElement);
...
export const getEligibleElementsForBinding = (
	...
	return elements.flatMap((element) =>
		isBindingElement(element, false)
			? (getElligibleElementsForBindingElement(element as NonDeleted<ExcalidrawLinearElement>).filter(
...
const getElligibleElementsForBindingElement = (
	...
	].filter((element): element is NonDeleted<ExcalidrawBindableElement> => element != null);
};
...
export const fixBindingsAfterDuplication = (
	...
	(sceneElements.filter(({ id }) => allBoundElementIds.has(id)) as ExcalidrawLinearElement[]).forEach(
		(element) => {
