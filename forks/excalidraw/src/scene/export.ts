...
import { Bounds, getCommonBounds, getElementAbsoluteCoords } from "../element/bounds";
import { renderSceneToSvg, renderStaticScene } from "../renderer/renderScene";
import { distance, getFontString } from "../utils";
import { AppState, BinaryFiles } from "../types";
import { DEFAULT_EXPORT_PADDING, FRAME_STYLE, SVG_NS, THEME_FILTER } from "../constants";
...
const __createSceneForElementsHack__ = (elements: readonly ExcalidrawElement[]) => {
...
export const exportToCanvas = async (
	...
	const { imageCache } = await updateImageCache({
		imageCache: new Map(),
		fileIds: getInitializedImageElements(nextElements).map((element) => element.fileId),
		files,
	});
	...
	renderStaticScene({
		...
		appState: {
			...appState,
			frameRendering: getFrameRenderingConfig(exportingFrame ?? null, appState.frameRendering ?? null),
...
export const exportToSvg = async (
	...
	for (const frame of frameElements) {
		...
		exportingFrameClipPath += `<clipPath id=${frame.id}>
            <rect transform="translate(${frame.x + offsetX} ${frame.y + offsetY}) rotate(${
							frame.angle
						} ${cx} ${cy})"
          width="${frame.width}"
          height="${frame.height}"
...
const getCanvasSize = (elements: readonly NonDeletedExcalidrawElement[], exportPadding: number): Bounds => {
