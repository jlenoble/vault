...
const bindLinearElementToElement = (
	...
	if (start) {
		...
		if (startType) {
			...
			bindLinearElement(linearElement, startBoundElement as ExcalidrawBindableElement, "start");
		}
	}
	if (end) {
		...
		if (endType) {
			...
			bindLinearElement(linearElement, endBoundElement as ExcalidrawBindableElement, "end");
		}
	}
...
export const convertToExcalidrawElements = (
	...
	for (const element of elements) {
		...
		switch (element.type) {
			...
			case "diamond": {
				const width =
					element?.label?.text && element.width === undefined ? 0 : element?.width || DEFAULT_DIMENSION;
				const height =
					element?.label?.text && element.height === undefined ? 0 : element?.height || DEFAULT_DIMENSION;
