...
import { fireEvent, GlobalTestState, render, screen, waitFor } from "./test-utils";
...
const createLinearElementWithCurveInsideMinMaxPoints = (type: "line" | "arrow", extraProps: any = {}) => {
...
const createLinearElementsWithCurveOutsideMinMaxPoints = (type: "line" | "arrow", extraProps: any = {}) => {
	return newLinearElement({
		type,
...
const checkRotatedHorizontalFlip = async (expectedAngle: number, toleranceInPx: number = 0.00001) => {
	const originalElement = cloneJSON(h.elements[0]);
	h.app.actionManager.executeAction(actionFlipHorizontal);
	const newElement = h.elements[0];
	await waitFor(() => {
...
describe("arrow", () => {
	...
	it("flips a rotated arrow horizontally with line inside min/max points bounds", async () => {
		...
		await checkRotatedHorizontalFlip(expectedAngle, MULTIPOINT_LINEAR_ELEMENT_FLIP_TOLERANCE_IN_PIXELS);
	});
	...
	it("flips a rotated arrow vertically with line inside min/max points bounds", async () => {
		...
		await checkRotatedVerticalFlip(expectedAngle, MULTIPOINT_LINEAR_ELEMENT_FLIP_TOLERANCE_IN_PIXELS);
	});
...
describe("mutliple elements", () => {
	it("with bound text flip correctly", async () => {
		...
		let editor = document.querySelector<HTMLTextAreaElement>(".excalidraw-textEditorContainer > textarea")!;
		...
		Keyboard.keyPress(KEYS.ENTER);
		editor = document.querySelector<HTMLTextAreaElement>(".excalidraw-textEditorContainer > textarea")!;
