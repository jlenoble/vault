...
export const getEmbedLink = (link: string | null | undefined): EmbeddedLink => {
	...
	if (vimeoLink?.[1]) {
		...
		const warning = !/^\d+$/.test(target) ? t("toast.unrecognizedLinkFormat") : undefined;
		type = "video";
...
export const isEmbeddableOrLabel = (element: NonDeletedExcalidrawElement): Boolean => {
...
export const createPlaceholderEmbeddableLabel = (element: ExcalidrawEmbeddableElement): ExcalidrawElement => {
	const text = !element.link || element?.link === "" ? "Empty Web-Embed" : element.link;
