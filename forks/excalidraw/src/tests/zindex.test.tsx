...
import { ExcalidrawElement, ExcalidrawFrameElement, ExcalidrawSelectionElement } from "../element/types";
...
describe("z-index manipulation", () => {
	...
	it("duplicating elements should retain zindex integrity", () => {
		...
		expect(h.elements).toMatchObject([{ id: "A" }, { id: "A_copy" }, { id: "B" }, { id: "B_copy" }]);
		...
		h.app.actionManager.executeAction(actionDuplicateSelection);
		expect(h.elements.map((element) => element.id)).toEqual(["A", "B", "A_copy", "B_copy", "C", "C_copy"]);
		...
		h.app.actionManager.executeAction(actionDuplicateSelection);
		expect(h.elements.map((element) => element.id)).toEqual(["A", "B", "C", "A_copy", "B_copy", "C_copy"]);
		...
		h.app.actionManager.executeAction(actionDuplicateSelection);
		expect(h.elements.map((element) => element.id)).toEqual(["A", "A_copy", "B", "C"]);
		...
		h.app.actionManager.executeAction(actionDuplicateSelection);
		expect(h.elements.map((element) => element.id)).toEqual(["A", "B", "B_copy", "C"]);
		...
		h.app.actionManager.executeAction(actionDuplicateSelection);
		expect(h.elements.map((element) => element.id)).toEqual(["A", "A_copy", "B", "B_copy", "C"]);
	});
	...
	it("duplicating incorrectly interleaved elements (group elements should be together) should still produce reasonable result", () => {
		...
		h.app.actionManager.executeAction(actionDuplicateSelection);
		expect(h.elements.map((element) => element.id)).toEqual(["A", "C", "A_copy", "C_copy", "B"]);
	});
