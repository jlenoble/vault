...
import { TransformHandleType, MaybeTransformHandleType, TransformHandleDirection } from "./transformHandles";
...
const rotateSingleElement = (
	...
	if (boundTextElementId) {
		const textElement =
			Scene.getScene(element)?.getElement<ExcalidrawTextElementWithContainer>(boundTextElementId);
...
export const resizeSingleElement = (
	...
	if (boundTextElement) {
		const stateOfBoundTextElementAtResize = originalElements.get(boundTextElement.id) as
			| typeof boundTextElement
			| undefined;
	...
	if ("scale" in element && "scale" in stateAtResizeStart) {
		mutateElement(element, {
			scale: [
				...
				(Math.sign(newBoundsX2 - stateAtResizeStart.x) || stateAtResizeStart.scale[0]) *
					stateAtResizeStart.scale[0],
				(Math.sign(newBoundsY2 - stateAtResizeStart.y) || stateAtResizeStart.scale[1]) *
					stateAtResizeStart.scale[1],
			],
		});
	}

	if (isArrowElement(element) && boundTextElement && shouldMaintainAspectRatio) {
...
export const resizeMultipleElements = (
	...
	const [anchorX, anchorY]: Point = shouldResizeFromCenter ? [midX, midY] : mapDirectionsToAnchors[direction];
	...
	const mapDirectionsToPointerPositions: Record<typeof direction, [x: boolean, y: boolean]> = {
	 ...
	 */
	const [flipFactorX, flipFactorY] = mapDirectionsToPointerPositions[direction].map((condition) =>
		condition ? 1 : -1,
	);
	...
	const elementsAndUpdates: {
		element: NonDeletedExcalidrawElement;
		update: Mutable<Pick<ExcalidrawElement, "x" | "y" | "width" | "height" | "angle">> & {
	...
	for (const { orig, latest } of targetElements) {
		...
		const boundTextElement = pointerDownState.originalElements.get(getBoundTextElementId(orig) ?? "") as
			| ExcalidrawTextElementWithContainer
			| undefined;
