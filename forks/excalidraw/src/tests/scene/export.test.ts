...
import { diamondFixture, ellipseFixture, rectangleWithLinkFixture } from "../fixtures/elementFixture";
...
describe("exportToSvg", () => {
	...
	it("with dark mode", async () => {
		...
		expect(svgElement.getAttribute("filter")).toMatchInlineSnapshot('"_themeFilter_f32792"');
	});
	...
	it("with elements that have a link", async () => {
		const svgElement = await exportUtils.exportToSvg([rectangleWithLinkFixture], DEFAULT_OPTIONS, null);
		expect(svgElement.innerHTML).toMatchSnapshot();
	});
});
...
describe("exporting frames", () => {
	...
	describe("exportToSvg", () => {
		...
		it("should render frame alone when not selected", async () => {
			...
			expect(svg.getAttribute("width")).toBe(frame.width.toString());
			expect(svg.getAttribute("height")).toBe((frame.height + getFrameNameHeight("svg")).toString());
		});
	});
});
