...
const changeFontSize = (
	...
	return {
		...
		appState: {
			...
			// the same font size
			currentItemFontSize:
				newFontSizes.size === 1 ? [...newFontSizes][0] : fallbackValue ?? appState.currentItemFontSize,
		},
			...
			<fieldset>
				...
				<ButtonIconSelect
					...
					options={[
						{
							value: "hachure",
							text: `${allElementsZigZag ? t("labels.zigzag") : t("labels.hachure")} (${getShortcutKey(
								"Alt-Click",
							)})`,
	...
	PanelComponent: ({ elements, appState, updateData }) => (
		<label className="control-label">
			...
			<input
				...
				value={
					getFormValue(elements, appState, (element) => element.opacity, true, appState.currentItemOpacity) ??
					undefined
				}
			/>
		</label>
	...
	perform: (elements, appState, value) => {
		return {
			elements: changeProperty(
				...
				(oldElement) => {
					if (isTextElement(oldElement)) {
						const newElement: ExcalidrawTextElement = newElementWith(oldElement, { textAlign: value });
						redrawTextBoundingBox(newElement, getContainerElement(oldElement));
						return newElement;
					}
	...
	perform: (elements, appState, value) => {
		return {
			elements: changeProperty(
				...
				(oldElement) => {
					if (isTextElement(oldElement)) {
						const newElement: ExcalidrawTextElement = newElementWith(oldElement, { verticalAlign: value });
...
export const actionChangeArrowhead = register({
	...
	trackEvent: false,
	perform: (elements, appState, value: { position: "start" | "end"; type: Arrowhead }) => {
		return {
			elements: changeProperty(elements, appState, (el) => {
