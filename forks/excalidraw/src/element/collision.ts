...
import { hasBoundTextElement, isEmbeddableElement, isImageElement } from "./typeChecks";
...
const isElementDraggableFromInside = (element: NonDeletedExcalidrawElement): boolean => {
	...
	const isDraggableFromInside =
		!isTransparent(element.backgroundColor) || hasBoundTextElement(element) || isEmbeddableElement(element);
...
export const isHittingElementBoundingBoxWithoutHittingElement = (
	...
	return (
		!isHittingElementNotConsideringBoundingBox(element, appState, frameNameBoundsCache, [x, y]) &&
...
const isElementSelected = (appState: AppState, element: NonDeleted<ExcalidrawElement>) =>
...
export const distanceToBindableElement = (element: ExcalidrawBindableElement, point: Point): number => {
	switch (element.type) {
		case "rectangle":
		case "image":
		case "text":
...
const distanceToDiamond = (element: ExcalidrawDiamondElement, point: Point): number => {
	const [, pointRel, hwidth, hheight] = pointRelativeToElement(element, point);
	const side = GALine.equation(hheight, hwidth, -hheight * hwidth);
	return GAPoint.distanceToLine(pointRel, side);
};

const distanceToEllipse = (element: ExcalidrawEllipseElement, point: Point): number => {
	const [pointRel, tangent] = ellipseParamsForTest(element, point);
	return -GALine.sign(tangent) * GAPoint.distanceToLine(pointRel, tangent);
};

const ellipseParamsForTest = (element: ExcalidrawEllipseElement, point: Point): [GA.Point, GA.Line] => {
	const [, pointRel, hwidth, hheight] = pointRelativeToElement(element, point);
...
const hitTestLinear = (args: HitTestArgs): boolean => {
	...
	if (args.check === isInsideCheck) {
		const hit = shape.some((subshape) =>
			hitTestCurveInside(subshape, relX, relY, element.roundness ? "round" : "sharp"),
		);
		if (hit) {
			return true;
		}
...
const relativizationToElementCenter = (element: ExcalidrawElement): GA.Transform => {
	const [x1, y1, x2, y2] = getElementAbsoluteCoords(element);
	const center = coordsCenter(x1, y1, x2, y2);
	// GA has angle orientation opposite to `rotate`
	const rotate = GATransform.rotation(center, element.angle);
...
const coordsCenter = (x1: number, y1: number, x2: number, y2: number): GA.Point => {
...
const getSortedElementLineIntersections = (
	...
	const sortedIntersections = intersections.sort(
		(i1, i2) => GAPoint.distance(i1, nearPoint) - GAPoint.distance(i2, nearPoint),
	);
...
const intersectSegment = (line: GA.Line, segment: [GA.Point, GA.Point]): GA.Point[] => {
	const [a, b] = segment;
...
const offsetSegment = (segment: [GA.Point, GA.Point], distance: number): [GA.Point, GA.Point] => {
	const [a, b] = segment;
...
export const getCircleIntersections = (center: GA.Point, radius: number, line: GA.Line): GA.Point[] => {
	if (radius === 0) {
...
const hitTestCurveInside = (drawable: Drawable, x: number, y: number, roundness: StrokeRoundness) => {
...
const hitTestRoughShape = (drawable: Drawable, x: number, y: number, lineThreshold: number) => {
