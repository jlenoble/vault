...
class FirebaseSceneVersionCache {
	...
	static set = (socket: SocketIOClient.Socket, elements: readonly SyncableExcalidrawElement[]) => {
		FirebaseSceneVersionCache.cache.set(socket, getSceneVersion(elements));
	};
}

export const isSavedToFirebase = (portal: Portal, elements: readonly ExcalidrawElement[]): boolean => {
		...
		[...new Set(filesIds)].map(async (id) => {
			try {
				...
				if (response.status < 400) {
					...
					const { data, metadata } = await decompressData<BinaryFileMetadata>(new Uint8Array(arrayBuffer), {
						decryptionKey,
					});
