		...
		if (idsOfElementsToDuplicate.get(element.id)) {
			...
			if (element.groupIds.length || boundTextElement || isElementAFrame) {
				...
				if (groupId) {
					...
					const groupElements = getElementsInGroup(sortedElements, groupId).flatMap((element) =>
						isFrameElement(element) ? [...getFrameChildren(elements, element.id), element] : [element],
					);
