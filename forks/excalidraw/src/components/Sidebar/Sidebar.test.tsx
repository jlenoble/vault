...
export const assertExcalidrawWithSidebar = async (
	...
	await render(<Excalidraw initialData={{ appState: { openSidebar: { name } } }}>{sidebar}</Excalidraw>);
	...
	describe("General behavior", () => {
		it("should render custom sidebar", async () => {
			const { container } = await render(
				<Excalidraw initialData={{ appState: { openSidebar: { name: "customSidebar" } } }}>
		...
		it("should toggle sidebar using props.toggleMenu()", async () => {
			...
			// -------------------------------------------------------------------------
			expect(window.h.app.toggleSidebar({ name: "customSidebar", force: false })).toBe(false);
			...
			// -------------------------------------------------------------------------
			expect(window.h.app.toggleSidebar({ name: "customSidebar", force: true })).toBe(true);
			expect(window.h.app.toggleSidebar({ name: "customSidebar", force: true })).toBe(true);
			...
			// -------------------------------------------------------------------------
			expect(window.h.app.toggleSidebar({ name: DEFAULT_SIDEBAR.name })).toBe(true);
