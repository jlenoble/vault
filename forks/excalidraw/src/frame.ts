...
export function isElementIntersectingFrame(element: ExcalidrawElement, frame: ExcalidrawFrameElement) {
...
export const getElementsCompletelyInFrame = (
	...
	omitGroupsContainingFrames(getElementsWithinSelection(elements, frame, false)).filter(
		(element) => (element.type !== "frame" && !element.frameId) || element.frameId === frame.id,
	);
...
export const elementOverlapsWithFrame = (element: ExcalidrawElement, frame: ExcalidrawFrameElement) => {
	return (
...
export const groupsAreAtLeastIntersectingTheFrame = (
	...
	return !!elementsInGroup.find(
		(element) => elementsAreInFrameBounds([element], frame) || isElementIntersectingFrame(element, frame),
	);
};
...
export const getFrameChildren = (allElements: ExcalidrawElementsIncludingDeleted, frameId: string) =>
	allElements.filter((element) => element.frameId === frameId);

export const getFrameElements = (
	allElements: ExcalidrawElementsIncludingDeleted,
): ExcalidrawFrameElement[] => {
	return allElements.filter((element) => isFrameElement(element)) as ExcalidrawFrameElement[];
};
...
export const getElementsInResizingFrame = (
	...
	const groupsToKeep = new Set<string>(
		Array.from(elementsCompletelyInFrame).flatMap((element) => element.groupIds),
	);
	...
	const individualElementsCompletelyInFrame = Array.from(elementsCompletelyInFrame).filter(
	...
	const newGroupElementsCompletelyInFrame = Array.from(elementsCompletelyInFrame).filter(
		(element) => element.groupIds.length > 0,
	);
...
export const getContainingFrame = (
	...
	if (element.frameId) {
		if (elementsMap) {
			return (elementsMap.get(element.frameId) || null) as null | ExcalidrawFrameElement;
		}
		return (Scene.getScene(element)?.getElement(element.frameId) as ExcalidrawFrameElement) || null;
	}
	return null;
};
 ...
 */
export const getTargetFrame = (element: ExcalidrawElement, appState: StaticCanvasAppState) => {
