...
import { fireEvent, mockBoundingClientRect, restoreOriginalGetBoundingClientRect } from "../tests/test-utils";
import { queryByText } from "@testing-library/react";

import { FONT_FAMILY, TEXT_ALIGN, VERTICAL_ALIGN } from "../constants";
import { ExcalidrawTextElement, ExcalidrawTextElementWithContainer } from "./types";
...
describe("textWysiwyg", () => {
	describe("start text editing", () => {
		...
		it("should prefer editing selected text element (non-bindable container present)", async () => {
			...
			expect((h.state.editingElement as ExcalidrawTextElement).containerId).toBe(null);
		});
	...
	describe("Test container-bound text", () => {
		...
		it("should bind text to container when triggered via context menu", async () => {
			...
			updateTextEditor(editor, "Excalidraw is an opensource virtual collaborative whiteboard");
			await new Promise((cb) => setTimeout(cb, 0));
			expect(h.elements.length).toBe(2);
			expect(h.elements[1].type).toBe("text");
		...
		it("should update font family correctly on undo/redo by selecting bounded text when font family was updated", async () => {
			...
			await new Promise((r) => setTimeout(r, 0));
			editor.blur();
			expect((h.elements[1] as ExcalidrawTextElementWithContainer).fontFamily).toEqual(FONT_FAMILY.Cascadia);

			//undo
			Keyboard.withModifierKeys({ ctrl: true }, () => {
				Keyboard.keyPress(KEYS.Z);
			});
			expect((h.elements[1] as ExcalidrawTextElementWithContainer).fontFamily).toEqual(FONT_FAMILY.Virgil);
      ...
      `);
		...
		it("should bind text correctly when container duplicated with alt-drag", async () => {
			...
			const duplicatedText = h.elements[1] as ExcalidrawTextElementWithContainer;
			...
			expect(duplicatedRectangle.boundElements).toStrictEqual([{ id: duplicatedText.id, type: "text" }]);
		...
		it("should reset the container height cache when font properties updated", async () => {
			...
			expect((h.elements[1] as ExcalidrawTextElementWithContainer).fontSize).toEqual(36);
		...
		it("should update line height when font family updated", async () => {
			...
			updateTextEditor(editor, "Hello World!");
			editor.blur();
			expect((h.elements[1] as ExcalidrawTextElementWithContainer).lineHeight).toEqual(1.25);
			...
			fireEvent.click(screen.getByTitle(/code/i));
			expect((h.elements[1] as ExcalidrawTextElementWithContainer).fontFamily).toEqual(FONT_FAMILY.Cascadia);
			expect((h.elements[1] as ExcalidrawTextElementWithContainer).lineHeight).toEqual(1.2);

			fireEvent.click(screen.getByTitle(/normal/i));
			expect((h.elements[1] as ExcalidrawTextElementWithContainer).fontFamily).toEqual(FONT_FAMILY.Helvetica);
			expect((h.elements[1] as ExcalidrawTextElementWithContainer).lineHeight).toEqual(1.15);
		});
		...
		it("shouldn't bind to container if container has bound text not centered and text tool is used", async () => {
			...
			await new Promise((r) => setTimeout(r, 0));
			updateTextEditor(editor, "Hello!");
			expect((h.elements[1] as ExcalidrawTextElementWithContainer).verticalAlign).toBe(VERTICAL_ALIGN.MIDDLE);
			...
			expect(rectangle.boundElements).toStrictEqual([{ id: text.id, type: "text" }]);
			expect((h.elements[1] as ExcalidrawTextElementWithContainer).verticalAlign).toBe(VERTICAL_ALIGN.BOTTOM);
