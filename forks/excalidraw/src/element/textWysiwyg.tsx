...
export const resetOriginalContainerCache = (id: ExcalidrawTextContainer["id"]) => {
	if (originalContainerCache[id]) {
		delete originalContainerCache[id];
	}
};

export const getOriginalContainerHeightFromCache = (id: ExcalidrawTextContainer["id"]) => {
...
export const textWysiwyg = ({
	...
	onSubmit: (data: { text: string; viaKeyboard: boolean; originalText: string }) => void;
	...
	const updateWysiwygStyle = () => {
		...
		const updatedTextElement = Scene.getScene(element)?.getElement<ExcalidrawTextElement>(id);
	...
	editable.onkeydown = (event) => {
		if (!event.shiftKey && actionZoomIn.keyTest(event)) {
			...
			event.key === KEYS.TAB ||
			(event[KEYS.CTRL_OR_CMD] && (event.code === CODES.BRACKET_LEFT || event.code === CODES.BRACKET_RIGHT))
		) {
			event.preventDefault();
	...
	const handleSubmit = () => {
		...
		// wysiwyg on update
		cleanup();
		const updateElement = Scene.getScene(element)?.getElement(element.id) as ExcalidrawTextElement;
		if (!updateElement) {
			return;
		}
				...
				const callback = (mutationList: MutationRecord[], observer: MutationObserver) => {
					const radixIsRemoved = mutationList.find(
						(mutation) =>
							...
							(mutation.removedNodes[0] as HTMLElement).dataset?.radixPopperContentWrapper !== undefined,
					);
	...
	const onPointerDown = (event: MouseEvent) => {
		const isTargetPickerTrigger =
			event.target instanceof HTMLElement && event.target.classList.contains("active-color");
