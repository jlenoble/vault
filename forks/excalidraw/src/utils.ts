...
import { DEFAULT_VERSION, EVENT, FONT_FAMILY, isDarwin, WINDOWS_EMOJI_FALLBACK_FONT } from "./constants";
...
): target is HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement | HTMLBRElement | HTMLDivElement =>
...
export const throttleRAF = <T extends any[]>(fn: (...args: T) => void, opts?: { trailing?: boolean }) => {
...
export const easeToValuesRAF = <T extends Record<keyof T, number>, K extends keyof T>({
	...
	function step(timestamp: number) {
		...
		Object.keys(fromValues).forEach((key) => {
			...
			const result = ((toValues[_key] - fromValues[_key]) * factor + fromValues[_key]) as T[keyof T];
...
export const isShallowEqual = <T extends Record<string, any>, I extends keyof T>(
	objA: T,
	objB: T,
...
export const isOnlyExportingSingleFrame = (elements: readonly NonDeletedExcalidrawElement[]) => {
	const frames = elements.filter((element) => element.type === "frame");

	return (
		frames.length === 1 &&
		elements.every((element) => element.type === "frame" || element.frameId === frames[0].id)
	);
};
