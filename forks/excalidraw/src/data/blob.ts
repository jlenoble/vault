...
export const parseLibraryJSON = (json: string, defaultStatus: LibraryItem["status"] = "unpublished") => {
...
export const ImageURLToFile = async (imageUrl: string, filename: string = ""): Promise<File | undefined> => {
	let response;
	try {
...
export const getFileFromEvent = async (event: React.DragEvent<HTMLDivElement>) => {
...
export const getFileHandle = async (
	...
	if (nativeFileSystemSupported) {
		try {
			...
			const handle: FileSystemHandle | null = (await (item as any).getAsFileSystemHandle()) || null;
...
const getActualMimeTypeFromImage = (buffer: ArrayBuffer) => {
	let mimeType: ValueOf<Pick<typeof MIME_TYPES, "png" | "jpg" | "gif">> | null = null;
