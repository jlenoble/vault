...
export class LinearElementEditor {
	...
	static handleBoxSelection(
		...
		const nextSelectedPoints = pointsSceneCoords.reduce((acc: number[], point, index) => {
			if (
				(point[0] >= selectionX1 &&
					point[0] <= selectionX2 &&
					point[1] >= selectionY1 &&
	...
	static handlePointDragging(
		...
		const draggingPoint = element.points[linearElementEditor.pointerDownState.lastClickedPoint] as
			| [number, number]
			| undefined;
		...
		if (selectedPointsIndices && draggingPoint) {
					...
					selectedPointsIndices.map((pointIndex) => {
						const newPointPosition =
							pointIndex === linearElementEditor.pointerDownState.lastClickedPoint
								...
								: ([element.points[pointIndex][0] + deltaX, element.points[pointIndex][1] + deltaY] as const);
						return {
							index: pointIndex,
			...
			if (isBindingElement(element, false)) {
				...
				if (firstSelectedIndex === 0) {
					coords.push(
						tupleToCoors(LinearElementEditor.getPointGlobalCoordinates(element, element.points[0])),
					);
				}
				...
				if (lastSelectedIndex === element.points.length - 1) {
					coords.push(
						tupleToCoors(
							LinearElementEditor.getPointGlobalCoordinates(element, element.points[lastSelectedIndex]),
						),
					);
				}
	...
	static handlePointerDown(
		...
		const nextSelectedPointsIndices =
			clickedPointIndex > -1 || event.shiftKey
				? event.shiftKey || linearElementEditor.selectedPointsIndices?.includes(clickedPointIndex)
	...
	static duplicateSelectedPoints(appState: AppState) {
		...
		const nextPoints = points.reduce((acc: Point[], point, index) => {
			...
			if (isSelected) {
				...
				acc.push(
					nextPoint ? [(point[0] + nextPoint[0]) / 2, (point[1] + nextPoint[1]) / 2] : [point[0], point[1]],
				);
