...
const createElement = (opts: { uid: string } | ElementLike) => {
	...
	if ("uid" in opts) {
		const match = opts.uid.match(/^(?:\((\^|\w+)\))?(\w+)(?::(\d+))?(?:\((\w+)\))?$/)!;
		parent = match[1];
		id = match[2];
...
const idsToElements = (ids: (Id | ElementLike)[], cache: Cache = {}): readonly ExcalidrawElement[] => {
...
const test = <U extends `${string}:${"L" | "R"}`>(
	...
	expect(cleanElements(remoteReconciled)).deep.equal(cleanElements(_target), "remote reconciliation");
	...
	if (bidirectional) {
		try {
			expect(
				cleanElements(reconcileElements(cloneJSON(__local), cloneJSON(__remote), {} as AppState)),
...
describe("elements reconciliation", () => {
	it("reconcileElements()", () => {
		...
		test(["A", "B", "C"], ["A", "B:2", "G"], ["A:L", "B:R", "G:R", "C:L"]);
		test(["A:2", "B:2", "C"], ["D", "B:1", "A:3"], ["B:L", "A:R", "C:L", "D:R"]);
		test(["A:2", "B:2", "C"], ["D", "B:2", "A:3", "C"], ["D:R", "B:L", "A:R", "C:L"]);
		test(
			["A", "B", "C", "D", "E", "F"],
			["A", "B:2", "X", "E:2", "F", "Y"],
		...
		test(["A", "B", "C"], ["(B)X", "(A)Y", "(Y)Z"], ["A:L", "B:L", "X:R", "Y:R", "Z:R", "C:L"]);

		test(["A"], ["(^)X", "Y"], ["X:R", "Y:R", "A:L"]);
		test(["A"], ["(^)X", "Y", "Z"], ["X:R", "Y:R", "Z:R", "A:L"]);

		test(["A", "B"], ["(A)C", "(^)D", "F"], ["A:L", "C:R", "D:R", "F:R", "B:L"]);

		test(["A", "B", "C", "D"], ["(B)C:1", "B", "D:1"], ["A:L", "C:R", "B:L", "D:R"]);

		test(["A", "B", "C"], ["(^)X", "(A)Y", "(B)Z"], ["X:R", "A:L", "Y:R", "B:L", "Z:R", "C:L"]);

		test(["B", "A", "C"], ["(^)X", "(A)Y", "(B)Z"], ["X:R", "B:L", "A:L", "Y:R", "Z:R", "C:L"]);
		...
		test(["C:1", "B", "D:1"], ["A", "B", "C:1", "D:1"], ["A:R", "B:L", "C:L", "D:L"]);

		test(["A", "B", "C", "D"], ["(A)C:1", "(C)B", "(B)D:1"], ["A:L", "C:R", "B:L", "D:R"]);

		test(["A", "B", "C", "D"], ["(A)C:1", "(C)B", "(B)D:1"], ["A:L", "C:R", "B:L", "D:R"]);

		test(["C:1", "B", "D:1"], ["(^)A", "(A)B", "(B)C:2", "(C)D:1"], ["A:R", "B:L", "C:R", "D:L"]);

		test(["A", "B", "C", "D"], ["(C)X", "(B)Y", "(A)Z"], ["A:L", "B:L", "C:L", "X:R", "Y:R", "Z:R", "D:L"]);

		test(["A", "B", "C", "D"], ["(A)B:1", "C:1"], ["A:L", "B:R", "C:R", "D:L"]);
		test(["A", "B", "C", "D"], ["(A)C:1", "B:1"], ["A:L", "C:R", "B:R", "D:L"]);
		test(["A", "B", "C", "D"], ["(A)C:1", "B", "D:1"], ["A:L", "C:R", "B:L", "D:R"]);
	...
	it("test identical elements reconciliation", () => {
		const testIdentical = (local: ElementLike[], remote: ElementLike[], expected: Id[]) => {
