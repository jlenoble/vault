...
import { convertToExcalidrawElements, Excalidraw } from "./packages/excalidraw/index";
...
describe("adding elements to frames", () => {
	...
	const reorderElements = <T extends { type: ElementType }>(els: readonly T[], order: ElementType[]) => {
	...
	const commonTestCases = async (func: typeof resizeFrameOverElement | typeof dragElementIntoFrame) => {
	...
	describe("resizing frame over elements", async () => {
		...
		it.skip("resizing over text containers and labelled arrows", async () => {
			await resizingTest("rectangle", ["frame", "rectangle", "text"], ["rectangle", "text", "frame"]);
			await resizingTest("rectangle", ["frame", "text", "rectangle"], ["rectangle", "text", "frame"]);
			await resizingTest("rectangle", ["rectangle", "text", "frame"], ["rectangle", "text", "frame"]);
			await resizingTest("rectangle", ["text", "rectangle", "frame"], ["rectangle", "text", "frame"]);
	...
	describe("dragging elements into the frame", async () => {
		...
		it("random order 01", () => {
			...
			h.elements = [frame1, rectangle4, rectangle1, rectangle3, frame3, rectangle2, frame2];
