...
export const Hyperlink = ({
	...
	const handleSubmit = useCallback(() => {
		...
		if (isEmbeddableElement(element)) {
			...
			if (!embeddableURLValidator(link, appProps.validateEmbeddable)) {
				...
				const ar = embedLink ? embedLink.aspectRatio.w / embedLink.aspectRatio.h : 1;
	...
	useEffect(() => {
		...
		const handlePointerMove = (event: PointerEvent) => {
			...
			const shouldHide = shouldHideLinkPopup(element, appState, [event.clientX, event.clientY]) as boolean;
			if (shouldHide) {
...
export const getContextMenuLabel = (elements: readonly NonDeletedExcalidrawElement[], appState: AppState) => {
	const selectedElements = getSelectedElements(elements, appState);
