...
export const actionSelectAllElementsInFrame = register({
	...
	perform: (elements, appState, _, app) => {
		...
		if (selectedFrame && selectedFrame.type === "frame") {
			const elementsInFrame = getFrameChildren(getNonDeletedElements(elements), selectedFrame.id).filter(
...
export const actionSetFrameAsActiveTool = register({
	...
	keyTest: (event) =>
		!event[KEYS.CTRL_OR_CMD] && !event.shiftKey && !event.altKey && event.key.toLocaleLowerCase() === KEYS.F,
});
