...
import { AppState, StaticCanvasAppState, BinaryFiles, Zoom, InteractiveCanvasAppState } from "../types";
...
const isPendingImageElement = (element: ExcalidrawElement, renderConfig: StaticCanvasRenderConfig) =>
...
const drawElementOnCanvas = (
	...
	context.globalAlpha = ((getContainingFrame(element)?.opacity ?? 100) * element.opacity) / 10000;
	switch (element.type) {
		case "rectangle":
		case "embeddable":
		case "diamond":
...
const drawElementFromCanvas = (
	...
	if (isArrowElement(element) && boundTextElement) {
		...
		const shiftX =
			tempCanvas.width / 2 - (boundTextCx - x1) * window.devicePixelRatio * zoom - offsetX - padding * zoom;

		const shiftY =
			tempCanvas.height / 2 - (boundTextCy - y1) * window.devicePixelRatio * zoom - offsetY - padding * zoom;
		...
		if (
			...
			const textElement = getBoundTextElement(element) as ExcalidrawTextElementWithContainer;
...
export const renderElementToSvg = (
	...
	switch (element.type) {
		...
		case "embeddable": {
			...
			const label: ExcalidrawElement = createPlaceholderEmbeddableLabel(element);
			renderElementToSvg(
				label,
				rsvg,
				root,
