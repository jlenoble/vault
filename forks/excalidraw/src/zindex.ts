...
const getTargetIndex = (
	...
	if (!containingFrame && (nextElement.frameId || nextElement.type === "frame")) {
...
const getTargetElementsMap = <T extends ExcalidrawElement>(elements: readonly T[], indices: number[]) => {
...
const shiftElementsByOne = (
	...
	const selectedFrames = new Set<ExcalidrawFrameElement["id"]>(
		indicesToMove.filter((idx) => elements[idx].type === "frame").map((idx) => elements[idx].id),
	);
...
function shiftElementsAccountingForFrames(
	...
	for (const element of allElements) {
		if (elementsToMove.has(element.id)) {
			if (isFrameElement(element) || (element.frameId && fullySelectedFrames.has(element.frameId))) {
