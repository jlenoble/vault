				...
				if (newContainer) {
					mutateElement(newContainer, {
						boundElements: (element.boundElements || [])
							.filter(
								(boundElement) =>
									boundElement.id !== newTextElementId && boundElement.id !== boundTextElementId,
							)
							.concat({
								type: "text",
								id: newTextElementId,
...
export const handleBindTextResize = (
	...
	let textElement = Scene.getScene(container)!.getElement(boundTextElementId) as ExcalidrawTextElement;
	if (textElement && textElement.text) {
		if (!container) {
			return;
		}

		textElement = Scene.getScene(container)!.getElement(boundTextElementId) as ExcalidrawTextElement;
		let text = textElement.text;
		let nextHeight = textElement.height;
		let nextWidth = textElement.width;
		const maxWidth = getBoundTextMaxWidth(container);
		const maxHeight = getBoundTextMaxHeight(container, textElement as ExcalidrawTextElementWithContainer);
		...
		if (nextHeight > maxHeight) {
			...
			const updatedY =
				...
				(transformHandleType === "ne" || transformHandleType === "nw" || transformHandleType === "n")
					? container.y - diff
					: container.y;
		...
		if (!isArrowElement(container)) {
			mutateElement(
				textElement,
				computeBoundTextPosition(container, textElement as ExcalidrawTextElementWithContainer),
			);
		}
	}
};
...
export const getApproxMinLineWidth = (font: FontString, lineHeight: ExcalidrawTextElement["lineHeight"]) => {
...
export const getBoundTextElementId = (container: ExcalidrawElement | null) => {
	return container?.boundElements?.length
		? container?.boundElements?.filter((ele) => ele.type === "text")[0]?.id || null
		: null;
};
...
export const getBoundTextElement = (element: ExcalidrawElement | null) => {
	...
	if (boundTextElementId) {
		return (
			(Scene.getScene(element)?.getElement(boundTextElementId) as ExcalidrawTextElementWithContainer) || null
		);
	}
	return null;
};
...
export const getContainerCenter = (container: ExcalidrawElement, appState: AppState) => {
	if (!isArrowElement(container)) {
		return {
...
export const getBoundTextElementOffset = (boundTextElement: ExcalidrawTextElement | null) => {
...
export const shouldAllowVerticalAlign = (selectedElements: NonDeletedExcalidrawElement[]) => {
...
export const suppportsHorizontalAlign = (selectedElements: NonDeletedExcalidrawElement[]) => {
	return selectedElements.some((element) => {
		const hasBoundContainer = isBoundToContainer(element);
		if (hasBoundContainer) {
			const container = getContainerElement(element);
...
export const isValidTextContainer = (element: { type: ExcalidrawElement["type"] }) =>
...
export const getBoundTextMaxWidth = (
	container: ExcalidrawElement,
	boundTextElement: ExcalidrawTextElement | null = getBoundTextElement(container),
) => {
