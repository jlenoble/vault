import { createPasteEvent, parseClipboard, serializeAsClipboardJSON } from "./clipboard";
...
describe("parseClipboard()", () => {
	it("should parse JSON as plaintext if not excalidraw-api/clipboard data", async () => {
		...
		text = "123";
		clipboardData = await parseClipboard(createPasteEvent({ types: { "text/plain": text } }));
