...
describe("duplicating multiple elements", () => {
	it("duplicateElements should clone bindings", () => {
		...
		// --------------------------------------------------------------------------
		expect(origElements.map((e) => e.type)).toEqual(clonedElements.map((e) => e.type));
		origElements.forEach((origElement, idx) => {
			...
			if ("endBinding" in origElement) {
				if (origElement.endBinding) {
					expect(origElement.endBinding.elementId).not.toBe((clonedElement as any).endBinding?.elementId);
				} else {
			...
			if ("startBinding" in origElement) {
				if (origElement.startBinding) {
					expect(origElement.startBinding.elementId).not.toBe((clonedElement as any).startBinding?.elementId);
				} else {
		...
		const clonedArrows = clonedElements.filter((e) => e.type === "arrow") as ExcalidrawLinearElement[];
	...
	it("should remove id references of elements that aren't found", () => {
		...
		const clonedElements = duplicateElements(origElements) as any as typeof origElements;
		const [clonedRectangle, clonedText1, clonedArrow1, clonedArrow2, clonedArrow3] = clonedElements;
	...
	describe("should duplicate all group ids", () => {
		it("should regenerate all group ids and keep them consistent across elements", () => {
			...
			const clonedElements = duplicateElements(origElements) as any as typeof origElements;
			const [clonedRectangle1, clonedRectangle2, clonedRectangle3] = clonedElements;
