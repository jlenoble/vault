...
import { embeddableURLValidator, extractSrc, getEmbedLink } from "../element/embeddable";
import { ContextMenu, ContextMenuItems, CONTEXT_MENU_SEPARATOR } from "./ContextMenu";
...
import { ExcalidrawElementSkeleton, convertToExcalidrawElements } from "../data/transform";
...
const ExcalidrawSetAppStateContext = React.createContext<React.Component<any, AppState>["setState"]>(() => {
...
class App extends React.Component<AppProps, AppState> {
	...
	private updateEmbeddableRef(id: ExcalidrawEmbeddableElement["id"], ref: HTMLIFrameElement | null) {
		if (ref) {
			this.iFrameRefs.set(id, ref);
		}
	}

	private getHTMLIFrameElement(id: ExcalidrawEmbeddableElement["id"]): HTMLIFrameElement | undefined {
	...
	private renderEmbeddables() {
		...
		const embeddableElements = this.scene
			...
			.filter(
				(el): el is NonDeleted<ExcalidrawEmbeddableElement> => isEmbeddableElement(el) && !!el.validated,
			);
		...
		return (
			<>
				{embeddableElements.map((el) => {
					...
					const isActive =
						this.state.activeEmbeddable?.element === el && this.state.activeEmbeddable?.state === "active";
					const isHovered =
						this.state.activeEmbeddable?.element === el && this.state.activeEmbeddable?.state === "hover";
					...
					return (
						<div
							...
							style={{
								...
								["--embeddable-radius" as string]: `${getCornerRadius(Math.min(el.width, el.height), el)}px`,
							}}
						>
							<div
								...
								>
									{this.props.renderEmbeddable?.(el, this.state) ?? (
										<iframe
											...
											srcDoc={embedLink?.type === "document" ? embedLink.srcdoc(this.state.theme) : undefined}
											src={embedLink?.type !== "document" ? embedLink?.link ?? "" : undefined}
		...
		get: (frameElement) => {
			...
			if (
				...
				if (frameNameDiv) {
					...
					const boxSceneTopLeft = viewportCoordsToSceneCoords({ clientX: box.x, clientY: box.y }, this.state);
										...
										<ExcalidrawElementsContext.Provider value={this.scene.getNonDeletedElements()}>
											<ExcalidrawActionManagerContext.Provider value={this.actionManager}>
												<LayerUI
													...
													showExitZenModeBtn={
														typeof this.props?.zenModeEnabled === "undefined" && this.state.zenModeEnabled
													}
												...
												>
													{this.props.children}
													{this.state.openDialog === "mermaid" && <MermaidToExcalidraw />}
												</LayerUI>
	...
	private isMobileBreakpoint = (width: number, height: number) => {
		return (
			width < MQ_MAX_WIDTH_PORTRAIT || (height < MQ_MAX_HEIGHT_LANDSCAPE && width < MQ_MAX_WIDTH_LANDSCAPE)
		);
	};
	...
	private removeEventListeners() {
		...
		document.removeEventListener(EVENT.GESTURE_START, this.onGestureStart as any, false);
		document.removeEventListener(EVENT.GESTURE_CHANGE, this.onGestureChange as any, false);
		document.removeEventListener(EVENT.GESTURE_END, this.onGestureEnd as any, false);
	...
	private addEventListeners() {
		...
		document.addEventListener(EVENT.GESTURE_START, this.onGestureStart as any, false);
		document.addEventListener(EVENT.GESTURE_CHANGE, this.onGestureChange as any, false);
		document.addEventListener(EVENT.GESTURE_END, this.onGestureEnd as any, false);
	...
	componentDidUpdate(prevProps: AppProps, prevState: AppState) {
		...
		if (
			...
			maybeBindLinearElement(
				...
				this.state,
				this.scene,
				tupleToCoors(LinearElementEditor.getPointAtIndexGlobalCoordinates(multiElement, -1)),
			);
		}
	...
	public pasteFromClipboard = withBatchedUpdates(async (event: ClipboardEvent) => {
		...
		if (data.errorMessage) {
			...
			if (
				...
				(/^(http|https):\/\/[^\s/$.?#].[^\s]*$/.test(maybeUrl) || getEmbedLink(maybeUrl)?.type === "video")
			) {
		...
		this.setState(
			{
				...
				openSidebar:
					this.state.openSidebar && this.device.editor.canFitSidebar && jotaiStore.get(isSidebarDockedAtom)
	...
	scrollToContent = (
		target: ExcalidrawElement | readonly ExcalidrawElement[] = this.scene.getNonDeletedElements(),
		opts?:
			| {
					fitToContent?: boolean;
					fitToViewport?: never;
	...
	private onKeyDown = withBatchedUpdates((event: React.KeyboardEvent | KeyboardEvent) => {
		...
		if (isArrowKey(event.key)) {
			const step =
				(this.state.gridSize && (event.shiftKey ? ELEMENT_TRANSLATE_AMOUNT : this.state.gridSize)) ||
	...
	private handleTextWysiwyg(
		...
		const updateElement = (text: string, originalText: string, isDeleted: boolean) => {
			this.scene.replaceAllElements([
	...
	private getElementsAtPosition(
		...
		return getElementsAtPosition(elements, (element) =>
			...
			return containingFrame && this.state.frameRendering.enabled && this.state.frameRendering.clip
		...
		if (!event[KEYS.CTRL_OR_CMD] && !this.state.viewModeEnabled) {
			...
			if (container) {
				if (
					...
					isHittingElementNotConsideringBoundingBox(container, this.state, this.frameNameBoundsCache, [
						sceneX,
						sceneY,
					])
				) {
	...
	private redirectToLink = (event: React.PointerEvent<HTMLCanvasElement>, isTouchScreen: boolean) => {
	...
	private getTopLayerFrameAtSceneCoords = (sceneCoords: { x: number; y: number }) => {
		const frames = this.scene
			.getNonDeletedFrames()
			.filter((frame) => isCursorInFrame(sceneCoords, frame as ExcalidrawFrameElement));
	...
	private handleCanvasPointerMove = (event: React.PointerEvent<HTMLCanvasElement>) => {
		...
		if (this.state.multiElement) {
			...
			if (lastPoint === lastCommittedPoint) {
				...
				if (
					distance2d(scenePointerX - rx, scenePointerY - ry, lastPoint[0], lastPoint[1]) >=
					LINE_CONFIRM_THRESHOLD
				) {
				...
				points.length > 2 &&
				lastCommittedPoint &&
				distance2d(scenePointerX - rx, scenePointerY - ry, lastCommittedPoint[0], lastCommittedPoint[1]) <
					LINE_CONFIRM_THRESHOLD
			) {
				setCursor(this.interactiveCanvas, CURSOR_TYPE.POINTER);
				mutateElement(multiElement, {
				...
				mutateElement(multiElement, {
					points: [
						...points.slice(0, -1),
						[lastCommittedX + dxFromLastCommitted, lastCommittedY + dyFromLastCommitted],
					],
				});
			}
		...
		if (this.hitLinkElement && !this.state.selectedElementIds[this.hitLinkElement.id]) {
			...
			if (
				...
				if (
					(hitElement || this.isHittingCommonBoundingBoxOfSelectedElements(scenePointer, selectedElements)) &&
					!hitElement?.locked
				) {
					if (
						hitElement &&
						isEmbeddableElement(hitElement) &&
						this.isEmbeddableCenter(hitElement, event, scenePointerX, scenePointerY)
					) {
						setCursor(this.interactiveCanvas, CURSOR_TYPE.POINTER);
						this.setState({
	...
	handleHoverSelectedLinearElement(
		...
		if (this.state.selectedLinearElement) {
			...
			if (
				isHittingElementNotConsideringBoundingBox(element, this.state, this.frameNameBoundsCache, [
					scenePointerX,
					scenePointerY,
				])
			) {
	...
	private handleCanvasPointerDown = (event: React.PointerEvent<HTMLElement>) => {
		...
		if (this.state.activeTool.type === "text") {
			...
			const pendingImageElement =
				this.state.pendingImageElementId && this.scene.getElement(this.state.pendingImageElementId);
	...
	private handleCanvasPointerUp = (event: React.PointerEvent<HTMLCanvasElement>) => {
		...
		if (this.device.editor.isMobile && clicklength < 300) {
			...
			if (
				isEmbeddableElement(hitElement) &&
				this.isEmbeddableCenter(hitElement, event, scenePointer.x, scenePointer.y)
			) {
		...
		if (this.hitLinkElement && !this.state.selectedElementIds[this.hitLinkElement.id]) {
			if (
				...
				!isPointHittingLinkIcon(this.hitLinkElement, this.state, [scenePointer.x, scenePointer.y])
			) {
	...
	private maybeCleanupAfterMissingPointerUp(event: React.PointerEvent<HTMLElement>): void {
	...
	private handleCanvasPanUsingWheelOrSpaceDrag = (event: React.PointerEvent<HTMLElement>): boolean => {
		if (
			!(
	...
	private initialPointerDownState(event: React.PointerEvent<HTMLElement>): PointerDownState {
		...
		return {
			...
			originInGrid: tupleToCoors(
				getGridPoint(origin.x, origin.y, event[KEYS.CTRL_OR_CMD] ? null : this.state.gridSize),
			),
	...
	private createImageElement = ({ sceneX, sceneY }: { sceneX: number; sceneY: number }) => {
		const [gridX, gridY] = getGridPoint(
			sceneX,
			sceneY,
			this.lastPointerDownEvent?.[KEYS.CTRL_OR_CMD] ? null : this.state.gridSize,
	...
	private handleLinearElementOnPointerDown = (
      ...
      values from appState. */
			...
			const [startArrowhead, endArrowhead] =
				elementType === "arrow" ? [currentItemStartArrowhead, currentItemEndArrowhead] : [null, null];
			...
			const element = newLinearElement({
				...
				opacity: this.state.currentItemOpacity,
				roundness:
					this.state.currentItemRoundness === "round" ? { type: ROUNDNESS.PROPORTIONAL_RADIUS } : null,
				startArrowhead,
				endArrowhead,
				locked: false,
				frameId: topLayerFrame ? topLayerFrame.id : null,
	...
	private getCurrentItemRoundness(
		elementType: "selection" | "rectangle" | "diamond" | "ellipse" | "embeddable",
	) {
	...
	private createFrameElementOnPointerDown = (pointerDownState: PointerDownState): void => {
		const [gridX, gridY] = getGridPoint(
			pointerDownState.origin.x,
			pointerDownState.origin.y,
			this.lastPointerDownEvent?.[KEYS.CTRL_OR_CMD] ? null : this.state.gridSize,
	...
	private maybeCacheReferenceSnapPoints(
		...
		if (
			...
			SnapCache.setReferenceSnapPoints(
				getReferenceSnapPoints(this.scene.getNonDeletedElements(), selectedElements, this.state),
			);
		}
	}
	...
	private maybeCacheVisibleGaps(
		...
		if (
			...
			SnapCache.setVisibleGaps(
				getVisibleGaps(this.scene.getNonDeletedElements(), selectedElements, this.state),
			);
		}
	}
	...
	private onKeyUpFromPointerDownHandler(pointerDownState: PointerDownState): (event: KeyboardEvent) => void {
		return withBatchedUpdates((event: KeyboardEvent) => {
	...
	private onPointerMoveFromPointerDownHandler(pointerDownState: PointerDownState) {
		return withBatchedUpdatesThrottled((event: PointerEvent) => {
			...
			if (
				...
				if (
					distance2d(pointerCoords.x, pointerCoords.y, pointerDownState.origin.x, pointerDownState.origin.y) <
					DRAGGING_THRESHOLD
				) {
					return;
				}
			...
			if (this.state.selectedLinearElement) {
				...
				if (
					LinearElementEditor.shouldAddMidpoint(this.state.selectedLinearElement, pointerCoords, this.state)
				) {
			...
			if (
				(hasHitASelectedElement || pointerDownState.hit.hasHitCommonBoundingBoxOfSelectedElements) &&
				...
				if (this.state.editingLinearElement) {
					...
					this.setState((prevState) => {
						const nextSelectedElementIds = {
							...
							...elementsWithinSelection.reduce((acc: Record<ExcalidrawElement["id"], true>, element) => {
								acc[element.id] = true;
								return acc;
							}, {}),
						};
	...
	private handlePointerMoveOverScrollbars(event: PointerEvent, pointerDownState: PointerDownState): boolean {
	...
	private onPointerUpFromPointerDownHandler(
		...
		return withBatchedUpdates((childEvent: PointerEvent) => {
			...
			const { draggingElement, resizingElement, multiElement, activeTool, isResizing, isRotating } =
				this.state;
			...
			this.setState({
				...
				editingElement:
					multiElement || isTextElement(this.state.editingElement) ? this.state.editingElement : null,
				snapLines: [],
			...
			if (isImageElement(draggingElement)) {
				...
				try {
					...
					this.setState(
						{
							selectedElementIds: makeNextSelectedElementIds({ [imageElement.id]: true }, this.state),
						},
						() => {
							this.actionManager.executeAction(actionFinalize);
						},
				...
				if (!pointerDownState.drag.hasOccurred && draggingElement && !multiElement) {
					mutateElement(draggingElement, {
						points: [
							...
							[pointerCoords.x - draggingElement.x, pointerCoords.y - draggingElement.y],
						],
					});
					this.setState({
					...
					if (this.state.selectedLinearElement && this.state.selectedLinearElement.isDragging) {
							...
							if (elements.length > 0) {
								...
								nextElements.forEach((element) => {
									if (
										...
										getElementsInGroup(nextElements, element.groupIds[element.groupIds.length - 1]).length < 2
									) {
										mutateElement(
											element,
											{
						...
						if (topLayerFrame && !this.state.selectedElementIds[topLayerFrame.id]) {
							const elementsToAdd = selectedElements.filter(
								(element) =>
									element.frameId !== topLayerFrame.id && isElementInFrame(element, nextElements, this.state),
							);
			...
			if (pointerDownState.resize.isResizing) {
				...
				for (const frame of selectedFrames) {
					nextElements = replaceAllElementsInFrame(
						nextElements,
						getElementsInResizingFrame(this.scene.getElementsIncludingDeleted(), frame, this.state),
						frame,
						this.state,
					);
				}
	...
	private restoreReadyToEraseElements = (pointerDownState: PointerDownState) => {
		const elements = this.scene.getElementsIncludingDeleted().map((ele) => {
	...
	private onImageAction = async ({ insertOnCanvasDirectly }: { insertOnCanvasDirectly: boolean }) => {
		try {
			...
			if (insertOnCanvasDirectly) {
				...
				this.setState(
					...
					() => {
						this.insertImageElement(imageElement, imageFile, /* showCursorImagePreview */ true);
					},
				);
			}
		} catch (error: any) {
	...
	private initializeImageDimensions = (imageElement: ExcalidrawImageElement, forceNaturalSize = false) => {
	...
	private maybeSuggestBindingAtCursor = (pointerCoords: { x: number; y: number }): void => {
	...
	private maybeSuggestBindingsForLinearElementAtCoords = (
		...
		const suggestedBindings = pointerCoords.reduce((acc: NonDeleted<ExcalidrawBindableElement>[], coords) => {
	...
	private maybeSuggestBindingForAll(selectedElements: NonDeleted<ExcalidrawElement>[]): void {
	...
	private handleInteractiveCanvasRef = (canvas: HTMLCanvasElement | null) => {
		...
		if (canvas !== null) {
			...
			this.interactiveCanvas?.removeEventListener(EVENT.WHEEL, this.handleWheel);
	...
	private maybeDragNewGenericElement = (
		...
		if (draggingElement.type === "selection" && this.state.activeTool.type !== "eraser") {
			...
			const image =
				isInitializedImageElement(draggingElement) && this.imageCache.get(draggingElement.fileId)?.image;
			...
			selectedFrames.forEach((frame) => {
				...
				if (transformHandleType) {
					if (transformHandleType.includes("w")) {
						elementsInFrame.forEach((element) => {
							mutateElement(element, {
								x: frame.x + (frameElementsOffsetsMap.get(frame.id + element.id)?.x || 0),
								y: frame.y + (frameElementsOffsetsMap.get(frame.id + element.id)?.y || 0),
							});
						});
					}
				...
				getElementsInResizingFrame(this.scene.getNonDeletedElements(), frame, this.state).forEach((element) =>
	...
	private savePointer = (x: number, y: number, button: "up" | "down") => {
		...
		const { x: sceneX, y: sceneY } = viewportCoordsToSceneCoords({ clientX: x, clientY: y }, this.state);
