...
export const LibraryDropdownMenuButton: React.FC<{
	...
	const onPublishLibSuccess = (data: { url: string; authorName: string }, libraryItems: LibraryItems) => {
	...
	const renderLibraryMenu = () => {
		return (
			<DropdownMenu open={isLibraryMenuOpen}>
				...
				>
					{!itemsSelected && (
						<DropdownMenu.Item onSelect={onLibraryImport} icon={LoadIcon} data-testid="lib-dropdown--load">
							{t("buttons.load")}
						</DropdownMenu.Item>
					)}
					{!!items.length && (
	...
	return (
		<div className={clsx("library-menu-dropdown-container", className)}>
			{renderLibraryMenu()}
			{selectedItems.length > 0 && <div className="library-actions-counter">{selectedItems.length}</div>}
