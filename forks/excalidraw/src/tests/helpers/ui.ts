...
import { API } from "./api";
import { isFrameElement, isLinearElement, isFreeDrawElement, isTextElement } from "../../element/typeChecks";
...
const getElementPointForSelection = (element: ExcalidrawElement): Point => {
	...
	const target: Point = [x + (isLinearElement(element) || isFreeDrawElement(element) ? 0 : width / 2), y];
	let center: Point;
...
const transform = (
	...
	if (elements.length === 1) {
		handleCoords = getTransformHandles(elements[0], h.state.zoom, "mouse")[handle];
	} else {
...
export class UI {
	...
	static async editText<T extends ExcalidrawTextElement | ExcalidrawTextContainer>(element: T, text: string) {
		const textEditorSelector = ".excalidraw-textEditorContainer > textarea";
		const openedEditor = document.querySelector<HTMLTextAreaElement>(textEditorSelector);
	...
	static queryContextMenu = () => {
		return GlobalTestState.renderResult.container.querySelector(".context-menu") as HTMLElement | null;
	};
}
